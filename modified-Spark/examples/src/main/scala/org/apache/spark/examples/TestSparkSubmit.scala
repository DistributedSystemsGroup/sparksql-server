...
object SimpleAppClient {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val people = sc.textFile(args{0}).map(_.split(",")).map(p => Person(p(0), p(1).trim.toInt))
    people.toDF().registerTempTable("people")
    //send table name to server in rddToDataFrameHolder / SQLContext
    //send sql query
    val teenagers = sqlContext.sql("SELECT name FROM people WHERE age >= 13 AND age <= 19")
    val table_query = "people-SELECT name FROM people WHERE age >= 13 AND age <= 19-"+args{0}
    val tmp = teenagers.map(t => "Name: " + t(0))

    tmp.saveAsTextFile("abcdef")

    val countsTest1 = sc.textFile(args{0}).flatMap(_.split(" ")).map((_, 1)).reduceByKey(_+_)
    //countsTest1.asInstanceOf[ShuffledRDD[_,_,_]].setAggregator(null)

    val jarSender : JarSubmitter = new JarSubmitter(src)
    val id : Integer = jarSender.send()

    val client: SparkSubmitter = new SparkSubmitter(countsTest1, table_query, sc, src, id)
    client.start()
  }
}
...
class JarSubmitter (src : CodeSource) {
  ...
  def sendAnonfun(out : OutputStream): Unit = {
    ...
    val path : String = "/Users/suwax/WORKSPACE-INTELLIJ/testSQLServer/SimpleAppClient/out/artifacts/untitled_2_jar/untitled-2.jar"
