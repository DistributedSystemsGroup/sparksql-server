...
import org.apache.spark.rdd.DeLabellingRDD
import org.apache.spark.rdd.DemuxRDD
import org.apache.spark.rdd.HadoopRDD
import org.apache.spark.rdd.LabellingRDD
import org.apache.spark.rdd.MuxRDD
...
object testMux {
  ...
  def main(args: Array[String]) {
    ...
    val countsTest = textFile.flatMap(_.split(" ")).map((_, 1))//.reduceByKey(_ + _)
    val countsTest2 = textFile.flatMap(_.split(" ")).map((_, 2))//.reduceByKey(_ + _)

    //add dags to an array of dag
    var rdds: Array[RDD[_]] = Array[RDD[_]]()
    rdds = rdds :+ countsTest :+ countsTest2

    //use to create a list of SparkHadoopWriter
    val noRdd = rdds.size
    ...
    for (i <- 0 to rdds.size - 1) {
      val res: Array[ShuffledRDD[_, _, _]] = findListShuffledRDD(rdds {i})
    ...
    for (i <- 0 to rdds.size - 1) {
      ...
      if (!rdds {i}.isInstanceOf[ShuffledRDD[_, _, _]]) {
        partI = partI :+ rdds {i}
        if (hasShuffledRDD(partI {partI.size - 1})) {
          testSetShuffleNull(partI {partI.size - 1})
        }
      }
      for (j <- 0 to listShuffle {i}.size - 1) {
        partI = partI :+ listShuffle {i} {j}.getPrev()
        listShuffle {i} {j}.setPrev(null)
        if (hasShuffledRDD(partI {partI.size - 1})) {
          testSetShuffleNull(partI {partI.size - 1})
        }
      }
    ...
    for (i <- 0 to parts.size - 1)
      firstParts = firstParts :+ parts {i} {parts {i}.size - 1}
    insertCommonMux2(firstParts)

    textFile.testMux.flatMap(_.split(" ")).map((_, 1)).collect.foreach(println)

    //firstParts{0}.collect.foreach(println)
    //firstParts{1}.collect.foreach(println)
    val labeled2 = firstParts {0}.label(firstParts)
    ...
    for (i <- 0 to listShuffle.size - 1)
      if (listShuffle {i}.size >= 1) {
        hasShuffled2 {i} = true
      }

    for (i <- 0 to parts.size - 1)
      parts {i} = parts {i}.dropRight(1)
