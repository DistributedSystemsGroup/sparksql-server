...
import org.apache.spark.sqlserver.submitter.{JarSubmitter, DAGSubmitter}
...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  // Set this Spark Application as Server or not
  private var isSparkSQLServer = false

  def getIsSparkSQLServer() : Boolean = isSparkSQLServer

  def setAsSparkSQLServer: Unit = {
    isSparkSQLServer = true
  }

  private var sqlQuery = "null"
  private var tableName = "null"

  def setSQLQuery(sql : String): Unit = {
    sqlQuery = sql
  }

  def getSQLQuery() : String = sqlQuery

  def setTableName(tn : String): Unit = {
    tableName = tn
  }
  ...
  def checkBroadCastInfoExisted(id : Long) : Boolean = {
    var res = true
    val map : TimeStampedHashMap[BlockId, BlockInfo] =  env.blockManager.getBlockInfo()
    res = map.contains(BroadcastBlockId(`id`, ""))
    res
  }
  ...
  def broadcastBySQLServer[T: ClassTag](value: T, id: Long): Broadcast[T] = {
    assertNotStopped()
    ...
    logInfo("Recreated broadcast " + bc.id + " from " + callSite.shortForm)
  ...
  def runJob[T, U: ClassTag](
    ...
    if(isSparkSQLServer) {
      ...
      //val res = conf.getOption("spark.sqlserver") match {
        //case Some(sqlServerStr) =>
      val sqlServer: Array[String] = conf.get("spark.sqlServer").split(",")

      val path: String = new File(new URI(conf.get("spark.primaryResource"))).getCanonicalPath
      ...
      val jarSender: JarSubmitter = new JarSubmitter(path, sqlServerIP, jarServerPort)
      val id: Integer = jarSender.send()
      val client: DAGSubmitter = new DAGSubmitter(rdd, "", this, id, sqlServerIP, sqlServerPort)
      client.start()
      /*  case None =>
          dagScheduler.runJob(rdd, cleanedFunc, partitions, callSite, allowLocal,
            resultHandler, localProperties.get)
