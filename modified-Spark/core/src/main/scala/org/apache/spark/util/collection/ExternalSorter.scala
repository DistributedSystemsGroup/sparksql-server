...
private[spark] class ExternalSorter[K, V, C](
    ...
    aggregators: Array[Aggregator[K, V, C]] = null)
  ...
  def insertAll(records: Iterator[_ <: Product2[K, V]]): Unit = {
    ...
    if (shouldCombine) {
      ...
      //  mergeVals = mergeVals :+ aggregators{i}.mergeValue

      //for(i<-0 to aggregators.size - 1)
      //  createCombiners =  createCombiners :+ aggregators{i}.createCombiner

      val mergeValue = aggregator.get.mergeValue
      //val mergeValue2 = aggregator2.get.mergeValue
      //val createCombiner2 = aggregator2.get.createCombiner
      ...
      //    if (hadValue) mergeVals{i}(oldValue, kv._2) else createCombiners{i}(kv._2)
      //  }
      //  updates = updates :+ update
      //}
      //  updates = updates :+ (hadValue: Boolean, oldValue: C) => {
      ...
      //val update2 = (hadValue: Boolean, oldValue: C) => {
      //  if (hadValue) mergeValue2(oldValue, kv._2) else createCombiner2(kv._2)
      //}
      ...
      while (records.hasNext) {
        ...
        //map.changeValue((getPartition(kv._1), kv._1), updates{signal})
        //if(kv._1.asInstanceOf[Tuple2[_,_]]._1 == "0")
        map.changeValue((getPartition(kv._1), kv._1), update)
        //else
        //  map.changeValue((getPartition(kv._1), kv._1), update2)
