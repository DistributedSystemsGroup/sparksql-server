...
private[spark] class ZippedPartitionsRDD2[A: ClassTag, B: ClassTag, V: ClassTag](
  ...
  override def compute(s: Partition, context: TaskContext): Iterator[V] = {
    ...
    var testRet : List[AnyRef] = List[AnyRef]()
    ...
    //null
    //val x = rdd1.firstParent.iterator(partitions(0), context)
    //val x = rdd1.dependencies{0}.rdd.iterator(partitions(0), context)
    //val y = x.next

    //rdd1.testCleanF.asInstanceOf[Function1].app
    //rdd1.asInstanceOf[MapPartitionsRDD].mapCleanF.apply()
   // f(rdd1.iterator(partitions(0), context), rdd2.iterator(partitions(0), context))
  }
