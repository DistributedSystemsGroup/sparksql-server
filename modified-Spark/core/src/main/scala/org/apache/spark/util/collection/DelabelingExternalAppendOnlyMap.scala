...
import org.apache.spark.util.collection.DelabelingExternalAppendOnlyMap.HashComparator
...
import org.apache.spark.{Aggregator, Logging, SparkEnv}
 ...
 */
@DeveloperApi
class DelabelingExternalAppendOnlyMap[K, V, C](
                                                createCombiners: Array[V => C],
                                                mergeVals: Array[(C, V) => C],
                                                mergeCombinerss: Array[(C, C) => C],
                                                //updates: Array[(Boolean, C) => C],
                                                //mergeCombinersArr : Array[(C,C) => C],
                                                serializer: Serializer = SparkEnv.get.serializer,
                                                blockManager: BlockManager = SparkEnv.get
                                                  .blockManager)
  ...
  private var updatess: Array[(Boolean, C) => C] = Array[(Boolean, C) => C]()
  ...
  def setUpdate(ups: Array[(Boolean, C) => C]) {
    updatess = ups
  }
  ...
  def insertAll(entries: Iterator[Product2[K, V]]): Unit = {
    ...
    while (entries.hasNext) {
      ...
      signal = curEntry._1.asInstanceOf[Tuple2[_, _]]._1.asInstanceOf[Int]
      //val signal = getDeppestTuple(curEntry.asInstanceOf[Tuple2[_,_]])._1.asInstanceOf[String]
      // .toInt
      currentMap.changeValue(curEntry._1, updates {
        signal
      })
      addElementsRead()
    }
    ...
    private def mergeIfKeyExists(key: K, baseCombiner: C, buffer: StreamBuffer): C = {
      ...
      while (i < buffer.pairs.length) {
        ...
        if (pair._1 == key) {
          ...
          return mergeCombinerss {
            signal
          }(baseCombiner, pair._2)
        }
        i += 1
      }
      baseCombiner
  ...
  private def hashKey(kc: (K, C)): Int = DelabelingExternalAppendOnlyMap.hash(kc._1)
}

private[spark] object DelabelingExternalAppendOnlyMap {
