...
import org.apache.spark.util.collection.IndexDelabelingExternalAppendOnlyMap.HashComparator
 ...
 */
@DeveloperApi
class IndexDelabelingExternalAppendOnlyMap[K, V, C](
                                                     createCombiner: V => C,
                                                     mergeValue: (C, V) => C,
                                                     mergeCombiners: (C, C) => C,
                                                     index: Integer,
                                                     serializer: Serializer = SparkEnv.get
                                                       .serializer,
  ...
  def insertAll(entries: Iterator[Product2[K, V]]): Unit = {
    ...
    while (entries.hasNext) {
      ...
      val signal = curEntry._1.asInstanceOf[Tuple2[_, _]]._1.asInstanceOf[String].toInt
      if (signal == index) {
        currentMap.changeValue(curEntry._1, update)
      }
      else {
        currentMap.update(curEntry._1, curEntry._2.asInstanceOf[C])
      }
      addElementsRead()
    }
  }
  ...
  private def hashKey(kc: (K, C)): Int = IndexDelabelingExternalAppendOnlyMap.hash(kc._1)
}

private[spark] object IndexDelabelingExternalAppendOnlyMap {
