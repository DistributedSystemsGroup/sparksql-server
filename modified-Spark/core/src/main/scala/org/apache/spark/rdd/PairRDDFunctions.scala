...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
   ...
   */
  def myCombineByKey[C](createCombiner: V => C,
                        mergeValue: (C, V) => C,
  ...
  def uniteShuffle[C](aggs: Array[Aggregator[K, V, C]],
                      partitioner: Partitioner = defaultPartitioner(self),
                      mapSideCombine: Boolean = true,
                      serializer: Serializer = null): RDD[(K, C)] = {
    if (keyClass.isArray) {
      if (mapSideCombine) {
    ...
    new UnitedShuffledRDD[K, V, C](self, partitioner)
      .setSerializer(serializer)
      .setAggregators(aggs)
      .setMapSideCombine(mapSideCombine)
  }

  def myReduceByKey(func: (V, V) => V, func2: (V, V) => V): RDD[(K, V)] = {
    myReduceByKey(defaultPartitioner(self), func, func2)
  }

  def myReduceByKey(partitioner: Partitioner, func: (V, V) => V,
                    func2: (V, V) => V): RDD[(K, V)] = {
    myCombineByKey[V]((v: V) => v, func, func, func2, func2, partitioner)
  }
  ...
  def myCombineByKey[C](createCombiner: V => C,
    ...
    val aggregator2 = new Aggregator[K, V, C](
      self.context.clean(createCombiner),
      self.context.clean(mergeValue2),
      self.context.clean(mergeCombiners2))
  ...
  def countApproxDistinctByKey(relativeSD: Double, partitioner: Partitioner): RDD[(K, Long)] = {
    ...
    assert(p <= 32)
    countApproxDistinctByKey(if (p < 4) {
      4
    } else {
      p
    }, 0, partitioner)
