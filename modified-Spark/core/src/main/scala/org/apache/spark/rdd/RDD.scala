...
import org.apache.spark.sqlserver.submitter.{DAGSubmitter, JarSubmitter}
...
abstract class RDD[T: ClassTag](
  ...
  def setSparkContext(ssc: SparkContext): Unit = {
    _sc = ssc
  }

  var testCleanF: AnyRef = null
  ...
  def map[U: ClassTag](f: T => U): RDD[U] = {
    ...
    testCleanF = cleanF
    new MapPartitionsRDD[U, T](this, (context, pid, iter) => iter.map(cleanF), cleanF)
  }

  def getDP: Seq[Dependency[_]] = deps

  def setDeps(sdeps: Seq[Dependency[_]]): Unit = {
    deps = sdeps
  }

  def getDP_(): Seq[Dependency[_]] = dependencies_

  def setDeps_(sdeps: Seq[Dependency[_]]): Unit = {
    dependencies_ = sdeps
  }

  def setCreationSite(): Unit = {
    creationSite = sc.getCallSite()
  }
  ...
  def filter2(f: T => Boolean): RDD[T] = new FilteredRDD(this, sc.clean(f))
  ...
  def mapPartitions[U: ClassTag](
    ...
    new MapPartitionsRDD(this, sc.clean(func), null, preservesPartitioning)
  }
  ...
  def label[U: ClassTag](rdds: Array[RDD[_]]): RDD[Tuple2[_, _]] = {
    new LabellingRDD(sc, rdds, preservesPartitioning = false)
  }

  def delabel[U: ClassTag](rdds: Array[RDD[_]]): RDD[Tuple2[_, _]] = {
    new DeLabellingRDD(sc, rdds, preservesPartitioning = false)
  }

  def packaging[U: ClassTag](other: RDD[U]): RDD[Tuple2[_, _]] = {
    packagingPartitions(other, preservesPartitioning = false) { (thisIter, otherIter) =>
  ...
  def packagingPartitions[B: ClassTag]
  (rdd2: RDD[B], preservesPartitioning: Boolean)
  (f: (Iterator[T], Iterator[B]) => Iterator[Tuple2[_, _]]): RDD[Tuple2[_, _]] =
    new PackagingRDD2(sc, sc.clean(f), this, rdd2, preservesPartitioning)

  def packagingReduce[U: ClassTag](other: RDD[U]): RDD[Tuple2[_, _]] = {
    packagingPartitionsReduce(other, preservesPartitioning = false) { (thisIter, otherIter) =>
      new Iterator[Tuple2[_, _]] {
        def hasNext = (thisIter.hasNext, otherIter.hasNext) match {
          case (true, true) => true
          case (false, false) => false
  ...
  def packagingPartitionsReduce[B: ClassTag]
  (rdd2: RDD[B], preservesPartitioning: Boolean)
  (f: (Iterator[T], Iterator[B]) => Iterator[Tuple2[_, _]]): RDD[Tuple2[_, _]] =
    new PackagingReduceRDD2(sc, sc.clean(f), this, rdd2, preservesPartitioning)

  def addToAggregatorArray[K, V, C](agg: Aggregator[K, V, C]): Unit = {
    if (this.isInstanceOf[ShuffledRDD[K, V, C]]) {
      this.asInstanceOf[ShuffledRDD[K, V, C]].addAggregator(agg)
    }
  }

  def setAggregators[K, V, C](aggs: Array[Aggregator[K, V, C]]): Unit = {
    if (this.isInstanceOf[ShuffledRDD[K, V, C]]) {
      this.asInstanceOf[ShuffledRDD[K, V, C]].setAggregators(aggs)
    }
  }
  ...
  def saveAsTextFile(path: String) {
    ...
    if(!_sc.getIsSparkSQLServer()) {
      val sqlServer: Array[String] = conf.get("spark.sqlServer").split(",")

      val oPath: String = new File(new URI(conf.get("spark.primaryResource"))).getCanonicalPath

      val sqlServerIP = sqlServer {0}

      val jarServerPort = sqlServer {2}.toInt

      val sqlServerPort = sqlServer {1}.toInt

      val jarSender: JarSubmitter = new JarSubmitter(oPath, sqlServerIP, jarServerPort)
      val id: Integer = jarSender.send()
      val tableName = sc.getTableName()
      val sqlQuery = sc.getSQLQuery()
      val input = "input"
      val output = path

      val metadata: String = conf.get("spark.metadata")

      val info: String = tableName + "__" + sqlQuery + "__" + input + "__" + output + "__" +
        metadata
      val client: DAGSubmitter = new DAGSubmitter(this, info, _sc, id, sqlServerIP, sqlServerPort)
      client.start()
    } else {
  ...
  @transient private[spark] var creationSite = sc.getCallSite()
