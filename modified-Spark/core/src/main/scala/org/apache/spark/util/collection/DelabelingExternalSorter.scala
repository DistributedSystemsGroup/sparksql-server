 ...
 *
 * @param aggregators Array of Aggregators with combine functions to use for merging data
 ...
 *                   want the output keys to be sorted. In a map task without map-side combine
 *                   for example, you
 ...
 *                   we sort elements of type ((Int, K), C) where the Int is the partition ID.
 *                   This is done to
 *                   avoid calling the partitioner multiple times on the same key (e.g. for
 *                   RangePartitioner).
 *
 *                   - When each buffer reaches our memory limit, we spill it to a file. This
 *                   file is sorted first
 *                   by partition ID and possibly second by key or by hash code of the key, if we
 *                   want to do
 *                   aggregation. For each file, we track how many objects were in each partition
 *                   in memory, so we
 *                   don't have to write out the partition ID for every element.
 *
 *                   - When the user requests an iterator, the spilled files are merged, along
 *                   with any remaining
 *                   in-memory data, using the same sort order defined above (unless both sorting
 *                   and aggregation
 ...
 *                   intermediate files.
 *
 *                   As a special case, if no Ordering and no Aggregator is given, and the number
 *                   of partitions is
 ...
 *                   then concatenate these files to produce a single sorted file, without having
 *                   to serialize and
 *                   de-serialize each item twice (as is needed during the merge). This speeds up
 *                   the map side of
 *                   groupBy, sort, etc operations since they do no partial aggregation.
 */
private[spark] class DelabelingExternalSorter[K, V, C](
                                                        partitioner: Option[Partitioner] = None,
                                                        ordering: Option[Ordering[K]] = None,
                                                        serializer: Option[Serializer] = None,
                                                        aggregators: Array[Aggregator[K, V, C]] =
                                                        null)
  ...
  private val bypassMergeSort =
    (numPartitions <= bypassMergeThreshold && (aggregators == null) && ordering.isEmpty)
  ...
  private val partitionKeyComparator: Comparator[(Int, K)] = {
    if (ordering.isDefined || aggregators != null) {
  ...
  def insertAll(records: Iterator[_ <: Product2[K, V]]): Unit = {
    ...
    val shouldCombine = (aggregators != null)
    ...
    if (shouldCombine) {
      ...
      for (i <- 0 to aggregators.size - 1)
        mergeVals = mergeVals :+ aggregators {
          i
        }.mergeValue

      for (i <- 0 to aggregators.size - 1)
        createCombiners = createCombiners :+ aggregators {
          i
        }.createCombiner
      ...
      while (records.hasNext) {
        ...
        if (kv.asInstanceOf[Tuple2[_, _]]._1.asInstanceOf[Tuple2[_, _]]._2 != null) {
          //signal = getDeppestTuple(kv.asInstanceOf[Tuple2[_,_]])._1.asInstanceOf[String].toInt
          signal = kv.asInstanceOf[Tuple2[_, _]]._1.asInstanceOf[Tuple2[_, _]]._1.asInstanceOf[Int]
          map.changeValue((getPartition(kv._1), kv._1), updates {
            signal
          })
  ...
  : Iterator[(Int, Iterator[Product2[K, C]])] = {
    ...
    (0 until numPartitions).iterator.map { p =>
      ...
      if (aggregators != null) {
        ...
        var mergeCombiners: Array[(C, C) => C] = Array[(C, C) => C]()
        for (i <- 0 to aggregators.size - 1)
          mergeCombiners = mergeCombiners :+ aggregators {
            i
          }.mergeCombiners

        (p, mergeWithAggregation(
          iterators, mergeCombiners, keyComparator, ordering.isDefined))
        ...
        override def next(): Iterator[Product2[K, C]] = {
          ...
          while (sorted.hasNext && comparator.compare(sorted.head._1, key) == 0) {
            ...
            while (i < keys.size && !foundKey) {
              if (keys(i) == pair._1) {
                //println(keys(i).toString)
                val signal = pair._1.asInstanceOf[Tuple2[_, _]]._1.asInstanceOf[Int]
                combiners(i) = mergeCombiners {
                  signal
                }(combiners(i), pair._2)
                foundKey = true
              }
      ...
      new Iterator[Product2[K, C]] {
        ...
        override def next(): Product2[K, C] = {
          ...
          while (sorted.hasNext && sorted.head._1 == k) {
            val pair = sorted.next()
            val signal = k.asInstanceOf[Tuple2[_, _]]._1.asInstanceOf[Int]
            c = mergeCombiners {
              signal
            }(c, pair._2)
          }
  ...
  def writePartitionedFile(
    ...
    if (bypassMergeSort && partitionWriters != null) {
      ...
      spillToPartitionFiles(if (aggregators != null) {
        map
      } else {
        buffer
      })
