...
class DAGContainer {
  ...
  private var info : DAGMetadata = new DAGMetadata()

  def setDAG(dg : RDD[_]) : Unit = {
    dag = dg
  }

  def setMetadata(ppb : DAGMetadata) : Unit = {
    info = ppb
  }

  def getDAG(): RDD[_] = dag

  def getMetadata() : DAGMetadata = info

  def updateMetadata(key : String, vl : Any) : Unit = {
